...
public class DureeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("sub").equals("calculer")) {
			...
			int repDuree = client.target("http://localhost:8020/REST/rest/credit/calculDuree").queryParam("capital", capitalValue).queryParam("annuitee", annuiteValue).queryParam("taux", tauxValue).request().get(Integer.class);			
