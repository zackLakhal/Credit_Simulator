...
public class CapitalServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("sub").equals("calculer")) {
			...
			double resp = client.target("http://localhost:8020/REST/rest/credit/calculCapital").queryParam("annuitee", annuiteValue).queryParam("duree", dureeValue).queryParam("taux", tauxValue).request().get(Double.class);			
	
			double repCapital = Math.round(resp);
